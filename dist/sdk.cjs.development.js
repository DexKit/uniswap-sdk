...
var isMATIC = function isMATIC(chainId) {
  switch (chainId) {
    case exports.ChainId.MATICMAINNET:
      return true;
...
var Currency = /*#__PURE__*/function () {
  ...
  Currency.getETHER = function getETHER(chainId) {
    if (isBSC(chainId)) {
...
var GET_ETHER = Currency.getETHER;
...
var CurrencyAmount = /*#__PURE__*/function (_Fraction) {
  ...
  CurrencyAmount.ether = function ether(amount, chainId) {
    return new CurrencyAmount(GET_ETHER(chainId), amount);
  };
...
var Pair = /*#__PURE__*/function () {
  ...
  Pair.getAddress = function getAddress(tokenA, tokenB) {
    ...
    // We assume is same chaind for both tokens

    var chainId = tokens[0].chainId;

    if (((_PAIR_ADDRESS_CACHE = PAIR_ADDRESS_CACHE) === null || _PAIR_ADDRESS_CACHE === void 0 ? void 0 : (_PAIR_ADDRESS_CACHE$t = _PAIR_ADDRESS_CACHE[tokens[0].address]) === null || _PAIR_ADDRESS_CACHE$t === void 0 ? void 0 : _PAIR_ADDRESS_CACHE$t[tokens[1].address]) === undefined) {
      var _PAIR_ADDRESS_CACHE2, _extends2, _extends3;

      PAIR_ADDRESS_CACHE = _extends({}, PAIR_ADDRESS_CACHE, (_extends3 = {}, _extends3[tokens[0].address] = _extends({}, (_PAIR_ADDRESS_CACHE2 = PAIR_ADDRESS_CACHE) === null || _PAIR_ADDRESS_CACHE2 === void 0 ? void 0 : _PAIR_ADDRESS_CACHE2[tokens[0].address], (_extends2 = {}, _extends2[tokens[1].address] = address.getCreate2Address(GET_FACTORY_ADDRESS(chainId), solidity.keccak256(['bytes'], [solidity.pack(['address', 'address'], [tokens[0].address, tokens[1].address])]), GET_INIT_CODE_HASH(chainId)), _extends2)), _extends3));
    }
  ...
  _proto.getOutputAmount = function getOutputAmount(inputAmount) {
    ...
    var outputReserve = this.reserveOf(inputAmount.token.equals(this.token0) ? this.token1 : this.token0); // Uniswap V2 and Matic is 997

    var inputAmountWithFee = JSBI.multiply(inputAmount.raw, isBSC(this.chainId) ? FEES_NUMERATOR : _997);
    var numerator = JSBI.multiply(inputAmountWithFee, outputReserve.raw); // Uniswap V2 and Matic is 1000

    var denominator = JSBI.add(JSBI.multiply(inputReserve.raw, isBSC(this.chainId) ? FEES_DENOMINATOR : _1000), inputAmountWithFee);
  ...
  _proto.getInputAmount = function getInputAmount(outputAmount) {
    ...
    var numerator = JSBI.multiply(JSBI.multiply(inputReserve.raw, outputAmount.raw), isBSC(this.chainId) ? FEES_DENOMINATOR : _1000);
    var denominator = JSBI.multiply(JSBI.subtract(outputReserve.raw, outputAmount.raw), isBSC(this.chainId) ? FEES_NUMERATOR : _997);
...
var Route = /*#__PURE__*/function () {
  function Route(pairs, input, output) {
    ...
    !(input instanceof Token && pairs[0].involvesToken(input) || input === GET_ETHER(pairs[0].chainId) && pairs[0].involvesToken(WETH[pairs[0].chainId])) ?  invariant(false, 'INPUT')  : void 0;
    !(typeof output === 'undefined' || output instanceof Token && pairs[pairs.length - 1].involvesToken(output) || output === GET_ETHER(pairs[0].chainId) && pairs[pairs.length - 1].involvesToken(WETH[pairs[0].chainId])) ?  invariant(false, 'OUTPUT')  : void 0;
...
function wrappedAmount(currencyAmount, chainId) {
  ...
  if (currencyAmount.currency === GET_ETHER(chainId)) return new TokenAmount(WETH[chainId], currencyAmount.raw);
    invariant(false, 'CURRENCY')  ;
}

function wrappedCurrency(currency, chainId) {
  if (currency instanceof Token) return currency;
  if (currency === GET_ETHER(chainId)) return WETH[chainId];
    invariant(false, 'CURRENCY')  ;
}
/**
...
var Trade = /*#__PURE__*/function () {
  function Trade(route, amount, tradeType) {
    ...
    var chainId = route.chainId;
    this.route = route;
    this.tradeType = tradeType;
    this.inputAmount = tradeType === exports.TradeType.EXACT_INPUT ? amount : route.input === GET_ETHER(chainId) ? CurrencyAmount.ether(amounts[0].raw, chainId) : amounts[0];
    this.outputAmount = tradeType === exports.TradeType.EXACT_OUTPUT ? amount : route.output === GET_ETHER(chainId) ? CurrencyAmount.ether(amounts[amounts.length - 1].raw, chainId) : amounts[amounts.length - 1];
...
var Router = /*#__PURE__*/function () {
  ...
  Router.swapCallParameters = function swapCallParameters(trade, options) {
    var etherIn = trade.inputAmount.currency === GET_ETHER(trade.route.chainId);
    var etherOut = trade.outputAmount.currency === GET_ETHER(trade.route.chainId); // the router does not support both ether in and out
...
}, _TOKEN_DECIMALS_CACHE[exports.ChainId.BSCMAINNET] = {
  '0xE0B7927c4aF23765Cb51314A0E0521A9645F0E2A': 9 // DGD
...
exports.FACTORY_ADDRESS_BSC = FACTORY_ADDRESS_BSC;
exports.Fetcher = Fetcher;
exports.Fraction = Fraction;
exports.GET_ETHER = GET_ETHER;
exports.GET_FACTORY_ADDRESS = GET_FACTORY_ADDRESS;
exports.GET_INIT_CODE_HASH = GET_INIT_CODE_HASH;
exports.INIT_CODE_HASH = INIT_CODE_HASH;
exports.INIT_CODE_HASH_BSC = INIT_CODE_HASH_BSC;
